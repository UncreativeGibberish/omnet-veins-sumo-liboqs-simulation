/*
 * PQCBaseApplLayer_hqc_slhdsa.cc
 *
...
#include "PQCBaseApplLayer_hqc_slhdsa.h"

//#include "veinspqcsim/ApplicationLayerTestMessage_m.h"
...
Define_Module(veinspqcsim::PQCBaseApplLayer_hqc_slhdsa);

void PQCBaseApplLayer_hqc_slhdsa::initialize( int stage )
{
    ...
    if ( stage == 0 ) {
        ...
        if( par("signatureAlgo").stdstringValue() == "SLHDSA" ){
            ...
            if (status != OQS_SUCCESS) {
                EV_ERROR << "Failed to generate SLH-DSA-shake-192f keypair" << endl;
            }
            else{
                EV_INFO << "SLH-DSA-shake-192f keypair generated. Public key size: "
...
void PQCBaseApplLayer_hqc_slhdsa::finish(){
    DemoBaseApplLayer::finish();
}

void PQCBaseApplLayer_hqc_slhdsa::onWSA( DemoServiceAdvertisment* wsa )
{
...
void PQCBaseApplLayer_hqc_slhdsa::onWSM( BaseFrame1609_4* frame )
{

    //RSU MESSAGE
    if( PQCRSUMessageWrapper_hqc_slhdsa* rsuMsg = dynamic_cast<PQCRSUMessageWrapper_hqc_slhdsa*>( frame ) ){
    ...
    PQCMessageWrapper_hqc_slhdsa* wsm = check_and_cast<PQCMessageWrapper_hqc_slhdsa*>( frame );
    ...
    if ( !sentMessage ) {
        ...
        PQCMessageWrapper_hqc_slhdsa* dupMsg = wsm->dup();
        dupMsg->setSerial( 2 );
...
void PQCBaseApplLayer_hqc_slhdsa::handleSelfMsg( cMessage* msg )
{
    if ( PQCMessageWrapper_hqc_slhdsa* wsm = dynamic_cast<PQCMessageWrapper_hqc_slhdsa*>( msg ) ) {
...
void PQCBaseApplLayer_hqc_slhdsa::handlePositionUpdate( cObject* obj )
{
    ...
    if ( mobility->getSpeed() < 1 ) {
        if ( simTime() - lastDroveAt >= 10 && sentMessage == false && kemPublicKeys.count(rsuAddr) > 0) {
            ...
            PQCMessageWrapper_hqc_slhdsa* wsm = new PQCMessageWrapper_hqc_slhdsa();

            populateWSM(wsm);
            printf("Returned from populateWSM");
            printf("Created PQCMessageWrapper_hqc_slhdsa, bitLength=%d, signature size=%zu\n",
...
void PQCBaseApplLayer_hqc_slhdsa::startService( Channel channel, int serviceId, std::string serviceDescription ){
    DemoBaseApplLayer::startService( channel, serviceId, serviceDescription );

}

void PQCBaseApplLayer_hqc_slhdsa::populateWSM( BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial ){
    // Todo: Clean up this entire function. Add more comments



    DemoBaseApplLayer::populateWSM( wsm, rcvId, serial );
    PQCMessageWrapper_hqc_slhdsa* msg = check_and_cast<PQCMessageWrapper_hqc_slhdsa*>(wsm);
    ...
    if ( !sigPublicKey.empty() ) {
        ...
        if(signatureAlgo == SigAlgo::SLH_DSA_192f){
            ...
            //More debug prints
            printf("DEBUG: slh-dsa- returned status = %d\n"
                    "signatureLength = %zu \n"
