/*
 * PQCBaseApplLayer_mlkem_mldsa.cc
 *
 *  Created on: 13 apr. 2026
 *      Author: Tomas Jonsson
 */

/* Todo: Implement new tbs here and in PQCRSU.
 * Implement handling of RSU messages
* Implement encapsulation and payload encryption for this module on RSU messages
* Implement decapsulation and payload decryption in PQCRSU
*/
#include "PQCBaseApplLayer_mlkem_mldsa.h"
#include "veinspqcsim/ApplicationLayerMessage_mlkem_mldsa_m.h"
//#include "RSUKemPubKeyAnnouncement_m.h"
...
Define_Module(veinspqcsim::PQCBaseApplLayer_mlkem_mldsa);

void PQCBaseApplLayer_mlkem_mldsa::initialize( int stage )
{
    ...
    recordScalar("carSpawnPosX", vPosition.x);
    recordScalar("carSpawnPosY", vPosition.y);
    if ( stage == 0 ) {
        recordScalar("TestRepetition", getEnvir()->getConfigEx()->getActiveRunNumber());
        sentMessage = false;
        lastDroveAt = simTime();
        ...
        if( par("kemAlgo").stdstringValue() == "MLKEM" ){
            ...
            if ( kem == nullptr ) {
                ...
                if (status != OQS_SUCCESS) {
                    printf("Failed to generate KEM keypair\n");
                    EV_ERROR << "Failed to generate KEM keypair\n";
                    kemAlgo = KemAlgo::None;
                }
                else{
                    EV_INFO << "ML-KEM-768 keypair generated successfully. "
                            << "PubKey size: " << kem->length_public_key
                            << ", SecretKey size: " << kem->length_secret_key << "\n";

                    printf("ML-KEM-768 keypair generated.\n"
                            "Public key size: %zu \n"
                            "Secret Key size: %zu",
...
void PQCBaseApplLayer_mlkem_mldsa::finish(){
/*
    cModule* mac = getParentModule()->getSubmodule("nic")->getSubmodule("mac1609_4");
    if ( mac ) {
        cStatistic* sentStat        = mac->getStatistic("sentPackets");
        cStatistic* collisionStat   = mac->getStatistic("collisions");
        cStatistic* busyStat        = mac->getStatistic("channelBusy");

        if (sentStat)       recordScalar("MAC_SentPackets",  sentStat->getSum());
        if (collisionStat)  recordScalar("MAC_Collisions",   collisionStat->getSum());
        if (busyStat)       recordScalar("MAC_ChannelBusy",  busyStat->getMean());   // timeavg → use getMean()
    }*/
    DemoBaseApplLayer::finish();
}

void PQCBaseApplLayer_mlkem_mldsa::onWSA( DemoServiceAdvertisment* wsa )
{
...
void PQCBaseApplLayer_mlkem_mldsa::onWSM( BaseFrame1609_4* frame )
{

    //RSU MESSAGE
    if( PQCRSUMessageWrapper_mlkem_mldsa* rsuMsg = dynamic_cast<PQCRSUMessageWrapper_mlkem_mldsa*>( frame ) ){
    ...
    PQCMessageWrapper_mlkem_mldsa* wsm = check_and_cast<PQCMessageWrapper_mlkem_mldsa*>( frame );
    ...
    if ( !sentMessage ) {
        sentMessage = true;
        // repeat the received traffic update once
        PQCMessageWrapper_mlkem_mldsa* dupMsg = wsm->dup();
        dupMsg->setSerial( 2 );
...
void PQCBaseApplLayer_mlkem_mldsa::handleSelfMsg( cMessage* msg )
{
    if ( PQCMessageWrapper_mlkem_mldsa* wsm = dynamic_cast<PQCMessageWrapper_mlkem_mldsa*>( msg ) ) {
        ...
        printf("Unexpected Message type in handleSelfMsg");
        DemoBaseApplLayer::handleSelfMsg( msg );
    }
}

void PQCBaseApplLayer_mlkem_mldsa::handlePositionUpdate( cObject* obj )
{
    ...
    if ( mobility->getSpeed() < 1 ) {
        if ( simTime() - lastDroveAt >= 10 && sentMessage == false && kemPublicKeys.count(rsuAddr) > 0) {
            ...
            PQCMessageWrapper_mlkem_mldsa* wsm = new PQCMessageWrapper_mlkem_mldsa();

            populateWSM(wsm);
            printf("Returned from populateWSM");
            printf("Created PQCMessageWrapper_mlkem_mldsa, bitLength=%d, signature size=%zu\n",
...
void PQCBaseApplLayer_mlkem_mldsa::startService( Channel channel, int serviceId, std::string serviceDescription ){
    DemoBaseApplLayer::startService( channel, serviceId, serviceDescription );

}

void PQCBaseApplLayer_mlkem_mldsa::populateWSM( BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial ){
    // Todo: Clean up this entire function. Add more comments



    DemoBaseApplLayer::populateWSM( wsm, rcvId, serial );
    PQCMessageWrapper_mlkem_mldsa* msg = check_and_cast<PQCMessageWrapper_mlkem_mldsa*>(wsm);
    ...
    Coord vPosition = mobility->getPositionAt( simTime() ); //get position at this moment in time
    recordScalar("carSendPosX", vPosition.x);
    recordScalar("carSendPosY", vPosition.y);
    // Set message fields
    ...
    if ( !sigPublicKey.empty() ) {
        ...
        printf("signature.size() == OQS_SIG_ml_dsa_65_length_signature: %d \n", (signature.size() == OQS_SIG_ml_dsa_65_length_signature));
        ...
        if(signatureAlgo == SigAlgo::MLDSA_65){
            ...
            //More debug prints
            printf("DEBUG: OQS_SIG_ml_dsa_65_sign returned status = %d\n"
                    "signatureLength = %zu \n"
