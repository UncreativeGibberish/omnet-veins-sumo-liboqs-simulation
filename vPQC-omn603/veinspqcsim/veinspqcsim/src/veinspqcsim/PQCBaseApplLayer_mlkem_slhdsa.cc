/*
 * PQCBaseApplLayerLayer_mlkem_slhdsa.cc
 *
 *  Created on: 8 juni 2026
 *      Author: Tomas Jonsson
 */

#include "PQCBaseApplLayer_mlkem_slhdsa.h"
#include "veinspqcsim/ApplicationLayerMessage_mlkem_slhdsa_m.h"
...
Define_Module(veinspqcsim::PQCBaseApplLayer_mlkem_slhdsa);

void PQCBaseApplLayer_mlkem_slhdsa::initialize( int stage )
{
    ...
    if ( stage == 0 ) {
        ...
        if( par("signatureAlgo").stdstringValue() == "SLHDSA" ){
            ...
            sig = OQS_SIG_new( OQS_SIG_alg_slh_dsa_pure_shake_192f );
             if ( sig == nullptr) {
                 printf( "Failed to create liboqs signature api instance" );
                 fflush(stdout);
             }
            ...
            if (status != OQS_SUCCESS) {
                EV_ERROR << "Failed to generate SLH-DSA keypair" << endl;
            }
            else{
                EV_INFO << "SLH-DSA keypair generated. Public key size: "
...
void PQCBaseApplLayer_mlkem_slhdsa::finish(){
    DemoBaseApplLayer::finish();
}

void PQCBaseApplLayer_mlkem_slhdsa::onWSA( DemoServiceAdvertisment* wsa )
{
...
void PQCBaseApplLayer_mlkem_slhdsa::onWSM( BaseFrame1609_4* frame )
{


    //RSU MESSAGE
    if( PQCRSUMessageWrapper_mlkem_slhdsa* rsuMsg = dynamic_cast<PQCRSUMessageWrapper_mlkem_slhdsa*>( frame ) ){
    ...
    PQCMessageWrapper_mlkem_slhdsa* wsm = check_and_cast<PQCMessageWrapper_mlkem_slhdsa*>( frame );
    ...
    //If the simulation is expanded, we might need to look into removing this penalty for all vehicles at some point
    if ( mobility->getRoadId()[0] != ':') traciVehicle->changeRoute(wsm->getRoadId(), 9999 );

    // If we haven't repeated this message. Do so
    if ( !sentMessage ) {
        sentMessage = true;
        PQCMessageWrapper_mlkem_slhdsa* dupMsg = wsm->dup();
        dupMsg->setSerial( 2 );
...
void PQCBaseApplLayer_mlkem_slhdsa::handleSelfMsg( cMessage* msg )
{
    if ( PQCMessageWrapper_mlkem_slhdsa* wsm = dynamic_cast<PQCMessageWrapper_mlkem_slhdsa*>( msg ) ) {
...
void PQCBaseApplLayer_mlkem_slhdsa::handlePositionUpdate( cObject* obj )
{
    DemoBaseApplLayer::handlePositionUpdate( obj );

    // stopped for for at least 10s?
    //if( simTime() - lastDroveAt >= 20 )
    if ( mobility->getSpeed() < 1 ) {
        if ( simTime() - lastDroveAt >= 10 && sentMessage == false && kemPublicKeys.count(rsuAddr) > 0) {
            findHost()->getDisplayString().setTagArg( "i", 1, "red" );
            sentMessage = true;

            PQCMessageWrapper_mlkem_slhdsa* wsm = new PQCMessageWrapper_mlkem_slhdsa();

            populateWSM(wsm);
            printf("Returned from populateWSM");
            printf("Created PQCMessageWrapper_mlkem_slhdsa, bitLength=%d, signature size=%zu\n",
...
void PQCBaseApplLayer_mlkem_slhdsa::startService( Channel channel, int serviceId, std::string serviceDescription ){
    DemoBaseApplLayer::startService( channel, serviceId, serviceDescription );

}

void PQCBaseApplLayer_mlkem_slhdsa::populateWSM( BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial ){
    // Todo: Clean up this entire function. Add more comments



    DemoBaseApplLayer::populateWSM( wsm, rcvId, serial );
    PQCMessageWrapper_mlkem_slhdsa* msg = check_and_cast<PQCMessageWrapper_mlkem_slhdsa*>(wsm);
    ...
    if ( !sigPublicKey.empty() ) {
        ...
        if(signatureAlgo == SigAlgo::SLH_DSA_192f){
            ...
            //More debug prints
            printf("DEBUG: Signing with OQS_SIG_alg_slh_dsa_pure_shake_192f returned status = %d\n"
                    "signatureLength = %zu \n"
